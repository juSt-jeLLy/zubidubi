...
- Playground page with live Graph market state, solver quotes, route split previews, shared-budget utilization/pressure badges, explicit skipped-maker reasons, protocol proof timeline, indexed activity, main proof catalog, copyable commands, GitHub proof links, latest proof status artifact, and a deploy-safe visual test map covering curve, routing, guardrail, receipt lifecycle, fork, and indexing proofs.
...
- **Shared term-risk budget executor deployed** (2026-09-12): `ZubiDubiRouteExecutor` now lets makers register one global receipt-exposure / quote-spend budget and assign sibling Aqua strategies to it. Quotes are capped by remaining budget, execution consumes budget immediately, and `pressurePenaltyBps` now actively reprices quotes as global budget utilization rises. The Graph indexes budget setup/use events, the solver enriches each route leg with normalized `budgetPressure` metadata, and the frontend Make/Portfolio/Sell/Playground surfaces show the shared reserve plus active pressure bps. Sepolia executor: `0x99488C09A54092Aa3C7e725137B45f3612CC5be1`.
...
- **Pendle benchmark path added and upgraded into a demo beat**: `npm run zubidubi:pendle-benchmark` compares the ZubiDubi routed quote for real `PT-USD3-17DEC2026` against Pendle RouterStatic's own `getPtToAssetRate(market)` value on mainnet fork. The current demo test prints Pendle's live implied PT rate, ZubiDubi's fresh-maker executable rate, ZubiDubi's post-inventory rate after the maker buys 200 PT, the signed bps delta versus Pendle, and the inventory penalty versus the fresh quote.
...
- **Frontend product shell added**: `frontend/` now has the finished dark DeFi app shell with navbar/footer, live Markets, live Sell route execution, live Make strategy shipping, live Portfolio issue/redeem/history flows, and a deployable Playground cockpit. The Playground now uses Graph market data and solver quote previews instead of static route cards; the local shell test runner remains a later optional dev-only layer.
- **Landing/markets split added**: `/` is now a dedicated ZubiDubi landing/narrative page, while `/markets` is the separate market-board surface for liquidity, discounts, maturities, and activity.
- **Solver/product API added**: `npm run zubidubi:solver-api` exposes `GET /health`, `GET /pitch`, `GET /markets`, and `GET|POST /quote`. The Railway deployment now runs this same API for the production frontend. It uses The Graph for market discovery and Sepolia `ZubiDubiRouteExecutor.quoteExactIn` for fresh executable route previews, so the deployed UI, CLI solver, and local dev flow share one backend path.
...
- **Subgraph `v0.9.6` deployed and verified**: endpoint `https://api.studio.thegraph.com/query/1760034/zubidubi/v0.9.6` indexes the new budget-aware route executor with `_meta.hasIndexingErrors = false`, shows the live multi-asset markets, and exposes the shared term-risk budget with `assignmentCount = 2` and `pressurePenaltyBps = 125`. The Graph-backed solver now defaults to live `PT-zbETH-30D -> USDC` and quotes `0.003 PT-zbETH-30D` at `7.439354 USDC` net from Graph-discovered strategy data and Sepolia RPC deliverability checks, including `budgetRemainingIn = 0.006`, `budgetRemainingOut = 50`, and `budgetPressure` metadata for route preview badges.
- **Solver API deployed to Railway**: `https://api-production-105bb.up.railway.app` runs the same Node API as local `npm run zubidubi:solver-api`, backed by Subgraph Studio `v0.9.6` and Sepolia RPC. Live `/health`, `/markets`, and `/quote?amountIn=0.003` smoke checks return executable route data with `canExecute = true`, shared-budget pressure metadata, and the exact `ZubiDubiRouteExecutor.routeExactIn` payload.
- **Public multi-maker book expanded** (2026-09-13): added `RunZubiDubiSepoliaMultiMakerBook`, funded two extra maker EOAs, and shipped six extra Sepolia Aqua strategies with maker-specific term curves and shared term-risk budgets. New active orders: `0xa51412cbb1973175537aa192e4d0cf7b4194486ea56b39244b93339dc3e58d19`, `0xb8c0957b983988f24293cd0546e1d68dc552220c564c2e5fb145d2362273e85d`, `0x34bec99a66ee0f2d9c2d1451a4513d852797f23062eb7206fee8240afefc6c9f`, `0xe55023b8ad014728faad9cf579802affdf126739ace71f2cb4f4b30d711c26bb`, `0x94a461a2ed50093c8f459aab9da276c3708e115dc4ed5c0d04ab8d607525016f`, and `0xc5b3dc9d3a760711a7565b4255a279da39957da0227794a6a5408a9d21e3cc84`. Graph `v0.9.6` indexes all six as `ACTIVE`; hosted `/quote` verifies `PT-zbETH-30D -> USDC` has three indexed strategies and executable multi-maker routes, while `PT-zbETH-30D -> WETH` fills through maker2.
- **Frontend deployed to Vercel production**: `https://frontend-gamma-jade-96.vercel.app` serves landing, Markets, Sell, Make, Portfolio, and Playground routes from the Vercel build output. Smoke checks returned HTTP 200 for `/`, `/markets`, `/sell`, `/portfolio`, `/make`, and `/playground`; the app is bundled against Subgraph Studio `v0.9.6` and the hosted Railway solver API.
- **Frontend Markets page connected to the live subgraph**: `/markets` now reads Subgraph Studio `v0.9.6` through a modular React Query data layer instead of the old hardcoded `MARKETS`/`ACTIVITY` arrays. Market cards, strategy counts, quote-token liquidity, maturity windows, activity feed, and the displayed block number now come from indexed Sepolia data and refresh every 15 seconds.
- **Frontend Sell page connected to the live solver API and wallet execution path**: `/sell` now discovers market pairs from the subgraph-backed market hook, lets the seller choose the actual payout market (`receipt -> USDC/WETH`), requests fresh Sepolia `quoteExactIn` route previews from the hosted Railway solver API, and blocks the quote flow when the solver reports partial/no fill. The API returns `422 insufficient_liquidity` with requested, available, and shortfall amounts instead of presenting an executable quote when maker liquidity cannot cover the input. Fully executable quotes include the exact route executor, raw amounts, decoded orders, and per-maker shared-budget pressure badges (`budget used` plus active pressure bps), so the connected Privy wallet submits the real `approve()` plus `ZubiDubiRouteExecutor.routeExactIn()` transaction path.
- **Sell page upgraded to a swap-style terminal**: `/sell` now presents the exit as a proper swap flow (`You sell` maturing claim -> `You receive` USDC/WETH), with a live quote button, route preview, maker fill bar, shared-budget pressure explanation, quote-health sidebar, settlement path, and transaction links. The root app shell is now flex-based so the footer stays below the main viewport instead of floating halfway up short pages.
...
- **Portfolio acquire surface connected to wallet issuance**: `/portfolio#acquire` now has a modular `AcquireDemoClaims` section backed by `services/portfolio/demoClaims.ts` and `services/portfolio/issueClaim.ts`. It builds the selectable claim list from live Graph receipt assets and indexed market routes, reads the connected wallet's WETH/USDC/LINK backing-token balance, previews `ZubiDubiExitReceipt.previewIssue()`, then submits real `approve(underlying -> receipt)` and `issue(assets, receiver)` transactions from the connected Privy wallet. The home page has a direct "Get demo assets" CTA into this section.
- **Portfolio page moved off mock data**: `/portfolio` now uses a modular live data stack instead of `HOLDINGS`, `MY_STRATEGIES`, `REDEMPTIONS`, and `ROUTE_PREVIEWS`. The page queries the deployed Subgraph Studio endpoint for wallet maker strategies, taker routes, route fills, fee events, markets, and receipt metadata; then reads the connected wallet's Sepolia receipt balances and `previewRedeem()` values directly from the receipt contracts. Matured positions show claim-ready status ticks and submit real `redeem(receiptAmount, receiver)` transactions from the connected Privy wallet, with confirmed claims inserted into the route/claim history while the subgraph catches up.
- **Frontend Make page connected to the live backend and contracts**: `/make` now reads market pairs from the Graph-backed market hook, builds maker strategies through the solver API instead of hardcoding SwapVM bytes in the UI, and ships the resulting strategy with the connected Privy wallet via real `approve(quoteToken -> Aqua)` plus `Aqua.ship(router, encodedOrder, tokens, amounts)` transactions. The solver API exposes `POST /strategies/build`, which selects the configured Sepolia Chainlink-backed ratio oracle for the receipt/payout pair, encodes the modular `BACKING_ORACLE_CHECK -> EXPOSURE_CAP -> DISCOUNT_CURVE_1D` opcode program, returns the order hash, and keeps opcode packing/ABI details out of the page. The Make page also shows the connected maker's indexed live strategies from The Graph after shipping.
- **Frontend Make curve editor upgraded**: `/make` now exposes the two real pricing axes behind the modular SwapVM strategy as a raw-SVG tabbed editor. The Time tab lets makers drag base discount, annualized term slope, max discount cap, convexity bow, and min/max maturity bounds; the Exposure tab lets makers drag inventory/liquidity penalty handles and max receipt exposure. Form fields, keyboard nudges, validation clamps, tooltips, and the `POST /strategies/build` payload all share one reducer-backed strategy state.
- **Sepolia inventory benchmark command added**: `npm run zubidubi:sepolia-inventory-benchmark` runs the focused fork proof for real Sepolia USDC plus the real Chainlink ETH/USD feed. Latest result: the same `0.0002 PT-zbETH` quote moves from `508095` USDC base units to `503204` after maker inventory rises to `0.0005 PT-zbETH`, a `96 bps` inventory penalty, and a `0.0004 PT-zbETH` follow-up quote is rejected because it would exceed the maker's `0.0008 PT-zbETH` exposure cap.
- **Sell quote benchmark panel added**: `/sell` now shows a benchmark card for every quote. It displays the executable route rate, a maturity-par comparison when the quote token matches the receipt backing, the fee-adjusted early-exit discount in bps, recent indexed market discount when available, and the solver's oracle/inventory benchmark note. This gives users context for what they are accepting before signing, and leaves a typed slot for a future live Pendle API benchmark while the mainnet-fork Pendle demo remains the verifiable external comparison.
- **Playground pressure visibility added**: route split previews now show the same shared-budget pressure metadata as Sell, including budget remaining, utilization percentage, and active pressure bps. This makes the differentiated budget-repricing mechanism visible during the judge demo instead of hiding it inside a worse quote.
...
- ~~Add a Pendle market benchmark panel to compare ZubiDubi maker quotes against Pendle's live implied PT pricing on the same asset.~~ **Contract/demo proof done** via `npm run zubidubi:pendle-benchmark`; frontend panel remains optional polish.
...
- Public solver API is deployed on Railway and still runnable locally with `npm run zubidubi:solver-api`; MCP remains optional polish.
- Public Sepolia now includes multiple maker EOAs. `RunZubiDubiSepoliaMultiMakerBook` shipped six extra live strategies from `0x59235DDD528B5970A3c7dFBa537C072a18B283e6` and `0xc1F20Cb0Ad8928Ed95bF18c5e9AC49dCd740542a`; Subgraph Studio `v0.9.6` indexes them as active with budget assignments, and the hosted solver quotes multi-maker `PT-zbETH-30D -> USDC` plus maker2-backed `PT-zbETH-30D -> WETH`.
...
- Pendle PT real-asset proof and benchmark script exist on mainnet fork. Latest judge-facing benchmark result: Pendle implied PT-to-asset rate `0.965766193005277136`, ZubiDubi fresh-maker PT-to-USDC rate `0.976660800000000000` (`+112 bps` versus Pendle for the seller), and ZubiDubi post-inventory rate `0.963875710000000000` (`-131 bps` versus the fresh maker quote), proving both external market benchmarking and inventory-aware repricing on the same real PT.
...
- The core live public maker demo is now in place: distinct maker EOAs, different risk curves, shared term-risk budgets, Graph-indexed assignments, and hosted API quotes that the frontend can consume.
- Remaining polish is submission/demo surface quality: walkthrough video, clean playground story, and optional dual-oracle live execution.
...
- Phase 12: `ZubiDubiRouteExecutor` now supports shared maker term-risk budgets. A maker can link sibling Aqua strategies to one `budgetId`, and the router caps quotes/execution by remaining global receipt exposure and quote-token spend so strategies cannot race the same reserve independently. It also applies a utilization-scaled pressure penalty to quotes, so a maker's sibling strategies become less generous as the shared reserve gets consumed instead of only stopping at a hard wall. The budget-aware executor is deployed on Sepolia at `0x99488C09A54092Aa3C7e725137B45f3612CC5be1` (tx `0xe8839d020ec655c3a914f5ba7a5c020d507e09afeeffb5277ea22a7b0cb3464f`, block `11685531`).
...
- The Graph: local subgraph added for live Sepolia indexing of Aqua strategies, SwapVM fills, ZubiDubi routes, maker skips, maker exposure, receipt maturity, shared term-risk budgets, budget consumption, and protocol/DAO fee accrual.
...
Current Sepolia deployment (budget-aware route executor upgrade, 2026-09-12, block 11685531):
...
- Deploy ZubiDubiRouteExecutor tx: `0xe8839d020ec655c3a914f5ba7a5c020d507e09afeeffb5277ea22a7b0cb3464f`.
...
- Live shared-budget proof: budget `0x25df91de3b0b88921665b1290762b8328e4239985cab7fb629b0d44fb642012a` was set in tx `0xbfeb5d8d2054ff1957e572149f1921b4c91fa9c25849416c3ccb09f28101a2ed`, then assigned to sibling `PT-zbETH-30D/USDC` and `PT-zbETH-180D/USDC` strategies in txs `0x8da5dc3ef79aa82ec157e0ee3247d810790da17eb4828d1b5c2aa99d7b32848c` and `0x67eb0bb26a3696f96b8beb761adfec6af0ca156fe0bc4ee63e37b9ca9c354d42`. Subgraph `v0.9.6` indexes `assignmentCount = 2` and `pressurePenaltyBps = 125`; the live solver quote returns `budgetRemainingIn = 0.006`, `budgetRemainingOut = 50`, and `budgetPressure` fields used by Sell/Playground to render budget utilization and active pressure bps.
...
- Fork proof result: sold `220 PT-USD3-17DEC2026` for `212.469044 USDC` net after DAO fee on the latest fork run.
- Demo benchmark result: Pendle implied rate `0.965766193005277136`; ZubiDubi fresh-maker rate `0.976660800000000000`; ZubiDubi post-inventory rate `0.963875710000000000`.
...
- Sibling strategies can share one maker term-risk budget, and a fill through one strategy immediately reduces the remaining quote capacity for all other strategies in that budget.
- If the candidate set cannot fill the requested amount, the whole route reverts.

Live Sepolia routed proof (historical hardened fixed-convexity route, block 11676033):

- Route executor used by that historical fill: `0x95d74BF2a83bc3ba50dc5c377cE8fB1478Ae5708`.
