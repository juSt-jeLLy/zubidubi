...
- Playground page with live Graph market state, solver quotes, route split previews, explicit skipped-maker reasons, protocol proof timeline, indexed activity, main proof catalog, copyable commands, GitHub proof links, latest proof status artifact, and a deploy-safe visual test map covering curve, routing, guardrail, receipt lifecycle, fork, and indexing proofs.
...
- **Sell page upgraded to a swap-style terminal**: `/sell` now presents the exit as a proper swap flow (`You sell` maturing claim -> `You receive` USDC/WETH), with a live quote button, route preview, maker fill bar, quote-health sidebar, settlement path, and transaction links. The root app shell is now flex-based so the footer stays below the main viewport instead of floating halfway up short pages.
